...
## 2026-06-29 — viral scraper pipeline finished: official IG hashtag API is App-Review-gated

- **The official Graph API hashtag path is dead for this app — `ig_hashtag_search` returns `OAuthException #10` ("Instagram Public Content Access" needs App Review).** Live-probed 2026-06-29 with the working token; confirms the HANDOFF §7 gotcha. So "discover top posts in ANY niche" (Elijah's #1 recurring ask) is impossible on the official API — it requires a scraper. Implication: don't re-attempt the official hashtag/top_media endpoints; the only ToS-clean options are a scraper (Apify/Bright Data) or staying watchlist-bound via `business_discovery`.
- **Built `intel/niche-radar.py`** (the discovery half): Apify `instagram-hashtag-scraper` → true view counts on arbitrary posts → ranks per niche → suggests winning NEW authors into `watchlist.json` (the hybrid: discovery feeds the official `business_discovery` tracking). Writes `niche-report.md` in viral-radar's exact line format, so `viral_teardown.py` (now reads BOTH reports) tears down discovered winners too. `Weekly Agent Refresh.bat` wires niche-radar→viral-radar→viral_teardown + the other orphaned engines (the level-up activation fix). Committed `922ff3d` (local, unpushed).
- **Apify gotcha for budget:** pay-per-RESULT (~$2.60/1k, ~$0.08 per 30-post hashtag); `resultsLimit` is PER hashtag (items = limit × #tags); the free tier returns only the FIRST PAGE and is a hard $5/mo wall; NEVER retry a 408 (the run keeps billing in the background). The engine has `--dry-run`, a `--results` clamp, and skips cleanly if `APIFY_TOKEN` is unset. **Pending Elijah:** create an Apify account → `setx APIFY_TOKEN "…"` → restart → run live.
- **Process learning:** an adversarial multi-lens review caught 4 real silent-data-loss bugs where niche-radar could emit a report line `viral_teardown`'s `POST_RE` rejects (empty permalink, apostrophe in owner name, negative comment count). Root cause: the self-test re-typed a COPY of the regex. Fix locked it by IMPORTING the real `POST_RE`. Implication: when two engines share a text contract, the test must import the real pattern, not a copy.
- **LIVE-CONFIRMED 2026-06-29 (APIFY_TOKEN set):** full chain runs end-to-end — 297 reels → 243 ranked → 24 suggested AI/founder accounts (e.g. @sabrina_ramonov, @rollins.io, @brycent), ~$0.77/run, inside the $5/mo free tier. Two live gotchas: (1) **`subprocess.run(text=True)` decodes curl output as Windows cp1252 → crashes on UTF-8 emoji bytes (0x8d) and silently empties stdout; fix = `encoding="utf-8", errors="replace"`** (same latent bug lurks in viral/competitor-radar — they just hit it less). (2) **Hashtag TOPIC-collisions are real:** `#saas` = Hindi "mother-in-law" (saas-bahu comedy dominates, NOT software); `#ceo`/`#software`/`#developer` pull multilingual noise — view counts can't filter wrong-topic content, only tag choice can. Refined to aitools/indiehackers/vibecoding/saasfounder. Cost research: NO safe free/local path (OSS needs login = ban risk to @elijahaifl); apidojo actor ~5× cheaper if scaling. Branch `claude/objective-greider-1516e5` pushed to origin; PR pending manual open (gh CLI not installed).
