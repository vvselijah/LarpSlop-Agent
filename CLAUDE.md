...
- **auto-clip status (2026-06-14):** FULL pipeline LIVE + tested — transcribe → agent-highlight → reframe/**facetrack** (face-track 9:16, lock-and-hold one speaker; tested on the Elijah/Derwin interview) → **tighten** (silence/filler cut) → **word-pop captions + audio cleanup** (libass). Remaining: LR-ASD active-speaker auto-switch (v2, gated torch), GPU torch-cu128, `Daily Agent Refresh.bat` wiring.
- **2026 analytics engines (new 2026-06-14, all stdlib/read-only, LOCAL/unpushed):** `ig-dashboard/metrics2026.py` (the 2026 IG-signal metric contract — skip/share/like/save/repost/comment rates, skip-gated score) · `ig-dashboard/watchtime_ideator.py` (watch-time arbitrage) · `intel/viral-radar.py` (competitor top-posts by views) · `intel/news-radar.py` (AI-news → content ideas) · **`content-intel-2026` skill** ("content briefing" → ties them together). Run via PowerShell. **Top follow-up (Elijah-gated):** add `reels_skip_rate`+`reposts` to `refresh.py` so metrics2026's skip-gate activates. Full handoff: `SESSION-COMPACT-2026-06-14.md`.
